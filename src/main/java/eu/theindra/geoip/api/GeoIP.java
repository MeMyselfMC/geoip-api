	...
	public GeoIP(InetAddress address) {
		if(GeoipAPI.databaseReader != null) {
			try {
				...
				Bukkit.getLogger().warning("Something went wrong while fetching GeoLite2 data for " + address.toString() + ".");
			}
		} else Bukkit.getLogger().info("Attempt to fetch GeoLite2 data for " + address.toString() + " failed because the database has not yet been initialized.");
	}
